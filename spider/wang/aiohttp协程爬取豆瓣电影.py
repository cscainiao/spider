...
class DouBanMovie():
    ...
    def __init__(self):
        # tag分类的url
        self.tag_url = 'http://movie.douban.com/j/search_tags?type=movie&source='
        # 获取电影的url
        self.movie_url = 'http://movie.douban.com/j/search_subjects?type=movie&tag=%s&sort=recommend&page_limit=20&page_start=%s'
        # 分类的信息
        self.tags = []
        # 查询分页的深度
        self.page = 10
    ...
    async def get_html_info(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self.tag_url, verify_ssl=False) as response:
            ...
            for tag in self.tags:
                for start_page in range(self.page):
                    async with session.get(self.movie_url % (tag, start_page*20), verify_ssl=False) as response:
